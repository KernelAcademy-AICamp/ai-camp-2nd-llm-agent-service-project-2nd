# Documentation Index

LEH 프로젝트의 모든 문서를 역할별로 정리했습니다.

## 문서 유형
- `[Human]` 개발자/팀원이 읽는 문서
- `[AI]` Claude Code 등 AI 에이전트용 문서
- `[Archive]` 완료된 리포트/스냅샷

---

## Specs & Design (`docs/specs/`) [Human]
- [PRD.md](specs/PRD.md): 제품 요구사항
- [ARCHITECTURE.md](specs/ARCHITECTURE.md): 시스템 아키텍처
- [BACKEND_DESIGN.md](specs/BACKEND_DESIGN.md): 백엔드 설계
- [FRONTEND_SPEC.md](specs/FRONTEND_SPEC.md): 프론트엔드 화면/UX
- [AI_PIPELINE_DESIGN.md](specs/AI_PIPELINE_DESIGN.md): AI 파이프라인
- [API_SPEC.md](specs/API_SPEC.md): REST API 명세
- [SECURITY_COMPLIANCE.md](specs/SECURITY_COMPLIANCE.md): 보안/규정
- [UAT_SCENARIOS.md](specs/UAT_SCENARIOS.md): UAT 시나리오

## Business & Ops (`docs/business/`) [Human]
- [COST_MODEL.md](business/COST_MODEL.md): 비용 모델
- [ROLLOUT_STRATEGY.md](business/ROLLOUT_STRATEGY.md): 출시 전략
- [TERMS_AND_PRIVACY.md](business/TERMS_AND_PRIVACY.md): 이용약관/개인정보

## Shared Guides (`docs/guides/`) [Human] - NEW
- [DESIGN_PATTERNS.md](guides/DESIGN_PATTERNS.md): 공통 디자인 패턴 (Strategy, Adapter, Factory, CQRS)
- [FOLDER_STRUCTURE.md](guides/FOLDER_STRUCTURE.md): 프로젝트 폴더 구조 (Backend, AI Worker, Frontend)
- [TESTING_STRATEGY.md](guides/TESTING_STRATEGY.md): 테스트 전략 통합 (TDD, Unit, Integration)

## Development Guides (`docs/guides/`) [Human]
- [CLEAN_ARCHITECTURE_GUIDE.md](guides/CLEAN_ARCHITECTURE_GUIDE.md): 아키텍처 원칙
- [BACKEND_SERVICE_REPOSITORY_GUIDE.md](guides/BACKEND_SERVICE_REPOSITORY_GUIDE.md): 백엔드 서비스/리포지토리
- [AI_WORKER_PATTERN_GUIDE.md](guides/AI_WORKER_PATTERN_GUIDE.md): AI 워커 패턴
- [FRONTEND_CLEAN_CODE.md](guides/FRONTEND_CLEAN_CODE.md): 프론트엔드 클린코드
- [UI_UX_DESIGN.md](guides/UI_UX_DESIGN.md): UI/UX 디자인 시스템
- [UI_IMPROVEMENTS.md](guides/UI_IMPROVEMENTS.md): Magic UI 통합 가이드

## AI Agent References (`docs/guides/`) [AI]
- [claude_example.md](guides/claude_example.md): AI 에이전트 규칙 (TDD, Tidy First)
- [plan.md](guides/plan.md): TDD 개발 계획 및 체크리스트
- [test_template.md](guides/test_template.md): 테스트 작성 템플릿
- [auto_test_clipping.md](guides/auto_test_clipping.md): 테스트 자동 필터링

## Environment & Operations [Human]
- [CONTRIBUTING.md](CONTRIBUTING.md): 협업/Git 워크플로우
- [ENVIRONMENT.md](ENVIRONMENT.md): 개발 환경 설정
- [OPERATIONS_PLAN.md](OPERATIONS_PLAN.md): 6주 운영 계획

## Root-Level Documentation
- [CLAUDE.md](../CLAUDE.md): Claude Code AI 에이전트 지침 [AI]
- [README.md](../README.md): 프로젝트 소개 [Human]

## Archive (`docs/archive/`) [Archive]
- [QA_REPORT_UI_IMPROVEMENTS.md](archive/QA_REPORT_UI_IMPROVEMENTS.md): UI 개선 QA (2025-11-24)
- [BUTTON_AUDIT_REPORT.md](archive/BUTTON_AUDIT_REPORT.md): 버튼 감사 (2025-11-24)
