...
### 3.8 회원가입 & 인증 고도화
- [x] 회원가입 페이지(/signup) 라우트를 추가한다.
- [x] 회원가입 UI에는 이메일, 비밀번호, 비밀번호 확인, 이름 입력 필드와 이용약관 동의 체크박스가 포함되어야 한다.
- [x] 각 필드에 대한 유효성 검사(Validation) 로직을 구현하고, 오류 메시지를 사용자에게 표시해야 한다.

### 3.9 Layout 고도화
- [x] 모든 페이지 하단에 일관된 Footer 컴포넌트를 구현한다.
- [x] Footer에는 법적 책임 고지(Disclaimer), 서비스 이용 약관, 개인정보 처리방침, 연락처, 사이트맵 링크가 포함되어야 한다.
