...
Windows file locking fix: ChromaDB holds SQLite locks that prevent cleanup.
Solution: Use gc.collect() + ignore_errors in shutil.rmtree
"""
...
def cleanup_chromadb(db_path: Path, store=None):
    """ChromaDB 정리 - Windows 파일 잠금 문제 해결"""
    # 1. VectorStore 참조 해제
    ...
    # 3. 파일 핸들 해제 대기 (Windows)
    time.sleep(0.1)

    # 4. 디렉토리 삭제 (ignore_errors로 실패해도 계속)
    if db_path.exists():
...
def temp_vector_db(tmp_path):
    ...
    yield str(db_path)
    # 테스트 후 정리 (ignore_errors 사용)
    cleanup_chromadb(db_path)
...
def vector_store(temp_vector_db):
    ...
    yield store
    # 정리: 클라이언트 참조 해제
    cleanup_chromadb(Path(temp_vector_db), store)
